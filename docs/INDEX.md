# Documentation Index

## Start here
- `../CLAUDE.md`
- `delivery/IMPLEMENTATION_BASELINE.md`
- `delivery/sprint-execution-model.md`
- `delivery/development-roadmap.md`

## Product
- `product/product-vision.md`
- `product/capabilities.md`
- `product/user-journey.md`

## Architecture
- `architecture/architecture-overview.md`
- `architecture/development-environment.md`
- `architecture/pipeline-architecture.md`
- `architecture/durable-execution.md`
- `architecture/evidence-model.md`

## Data
- `data/domain-model.md`
- `data/persistence-model.md`
- `data/atc-import-contract.md`

## AI
- `ai/ai-architecture.md`
- `ai/chat-orchestration.md`

## Integrations
- `integrations/sap-knowledge-mcp.md`

## UX
- `ux/app-shell.md`
- `ux/navigation-views.md`
- `ux/visual-design.md`

## Delivery
- `delivery/IMPLEMENTATION_BASELINE.md`
- `delivery/sprint-execution-model.md`
- `delivery/development-roadmap.md`
- `delivery/EXECUTION_STATE.yaml`
- `delivery/SESSION_HANDOFF.md`
- `delivery/BACKLOG.md`
- `delivery/SPRINT-PROGRESS.template.yaml`
- `delivery/SPRINT-RESULT.template.md`
- `delivery/results/`
- `delivery/sprints/`

## Claude Code commands
- `../.claude/commands/clean-core-status.md`
- `../.claude/commands/clean-core-run-sprint.md`
- `../.claude/commands/clean-core-review-sprint.md`
- `../.claude/commands/clean-core-validate.md`
- `../.claude/commands/clean-core-finish-sprint.md`

## Legacy
- `legacy/current-solution-assessment.md`


## Current baseline correction
- `adr/015-assessment-centric-workspace.md` — Client → Assessment, Assessments Home, conditional sidebar and permanent Copilot.
- `adr/016-variable-atc-xlsx-import.md` — schema-tolerant ATC XLSX ingestion with raw evidence preservation.
- `delivery/CLAUDE_CODE_ALIGNMENT_PROMPT.md` — historical/completed alignment prompt that produced Sprint 04; not the next execution instruction.
