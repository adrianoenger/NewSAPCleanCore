...
| BL-003 | Wrap future AI-processing stages (SPRINT-08+) in the same PipelineRun/StageRun/WorkItem engine | The engine in `backend/src/pipeline/` is generic (stage handlers registered via `StageDefinition`); object understanding, business-rule discovery, etc. should register as additional stages instead of building a parallel execution mechanism. | SPRINT-06 design | Medium | SPRINT-08+ | Open |
| BL-004 | Re-correlate ATC findings after Step 3 reprocessing | `ATCFinding.correlated_object_id` references `SAPObject.id`, but the `parse` stage deletes/recreates `SAPObject` rows per `source_file_id` on every run. Making "reprocess" a first-class Step 3 UI action (SPRINT-07) increases the chance that a reprocess silently breaks existing ATC↔object correlations. Needs a decision on stable object identity or a re-correlation pass after parse. | SPRINT-07 design | Medium | Future | Open |
